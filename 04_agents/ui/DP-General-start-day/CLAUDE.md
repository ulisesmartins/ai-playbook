...
change_summary: "Initial version — UI daily start-of-day orientation with active work surface, feedback queue, and review prep"
status: GA
audience: UI
---
...
Orients the designer for the day in under 5 minutes. Loads prior context, surfaces active UI Studio work and pending feedback, flags designs awaiting engineering handoff, and generates review prep for any scheduled design critiques or gate reviews.
...
| Active design work | ✅ | Feature names, UI Studio file names, or PRD references |
| Feedback received since last session | Optional | Free text or paste from UI Studio comments / Slack |
| Today's meetings | Optional | UI crits, gate reviews, handoff calls |
...
6. **Generate review prep** — For each scheduled meeting:
   - UI crit: list the 2–3 decisions the designer wants feedback on; describe what feedback is NOT needed (scope the critique)
   - Gate review: load `03_skills/cross-functional/DP-General-gate-review-checklist.md` and run a pre-flight check
   - Coding handoff: list components, states, and interactions to walk through
...
Date: [YYYY-MM-DD]
Active design work: [feature names or UI Studio file names]
...
| UI system gaps not surfaced | Component library not loaded | Always load component library at step 1 before reviewing active work |
