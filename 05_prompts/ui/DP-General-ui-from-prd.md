...
stakeholder review. Default tool is UI Studio — the agent selects the appropriate tool based on
...
```
You are a Dialpad UI Engineer building a prototype for Gate 1 review.
...
Prototype tool: [UI Studio / Bolt / v0.dev / "agent chooses"]
...
If using UI Studio: Output as a UI Studio prompt block ready to paste.
...
3. UI decisions log (any Dialtone token mappings or component choices that require PM/UI review)

---

## Known Limitations

- Generated prototypes are not production-ready — requires UI review before sharing externally
- Dialtone component availability varies by tool — some components may be approximated
- Cannot access live UI Studio — hand off to UI for high-fidelity UI Studio translation
