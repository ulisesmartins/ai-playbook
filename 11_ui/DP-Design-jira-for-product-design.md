...
- Have a single source of truth for the UI Team to have a bird-eye view on all in-flight and backlog projects
...
```sql
project IN (Dialpad, "Dialpad Meetings", "Mobile", "Dialtone", "Dialpad UI")
AND ((assignee IN (membersOf("Product UI")) OR labels IN (design-required)))
AND issuetype != Epic
```
...
| **Story / UI Story** | Small portion of work | Defined | <= Sprint |
...
!!! info "Source"
    ...
    This content is sourced from [Jira for Product UI](https://dialpad.atlassian.net/wiki/spaces/DES/pages/656801793) on Confluence.
