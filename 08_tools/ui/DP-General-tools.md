...
change_summary: "Added Dialpad UI Studio as the highest-fidelity prototyping option for UXEs; updated tool selection guide to distinguish UI Studio (real Dialtone components) from UI Studio (AI-generated approximation)"
status: GA
audience: UI
---
...
UI Studio + Claude Code layer on for structured design workflows.
...
**Best for:** UI brief generation, component spec drafting, design critique, Gate 1 prep
...
UI Studio prototypes render actual Dialtone components (`DtButton`, `DtInput`, `DtModal`, etc.) with real tokens and interactions. Unlike UI Studio or v0.dev, there is no AI approximation — the output looks and behaves exactly like the production product.

**When to use UI Studio:**
- Gate 1 or Gate 2 reviews where Coding will build directly from the prototype
...
### 5. UI Studio (with AI plugins)
**Best for:** High-fidelity UI design, component work in Dialtone, prototype annotation,
and AI-powered layout assistance

UI Studio is Dialpad's primary design tool. Key AI-assisted workflows:

- **UI Studio AI plugins** — Auto-layout suggestions, content generation, accessibility checks.

**Dialtone UI Studio Libraries to enable (DT9):**
...
### 5. Maze / UserTesting
**Best for:** Unmoderated usability testing on UI Studio prototypes
...
| Create a prototype (Gate 0.5, Gate 1, Gate 2) | **UI Studio** (UXE required) — https://dialpad.github.io/design-studio/#/ |
| Build high-fidelity screens | UI Studio + Dialtone DT9 libraries |
| Critique a design against Dialtone | Claude Code + `/design-critique` |
| Write component spec for Coding | Claude Code + `/component-spec` |
...
## Dialtone UI Rules (Always Active)

1. Every color must map to a Dialtone semantic token — never use hardcoded hex
2. Use DT9 components from the UI Studio library — do not create one-offs that duplicate existing components
3. All interactive elements require accessible names (WCAG AA minimum)
4. Annotate designs with Dialtone component names before Coding handoff
...
2. Enable Dialtone DT9 libraries in your UI Studio team
...
6. For high-fidelity prototypes, contact your UXE to set up UI Studio: https://dialpad.github.io/design-studio/#/
