...
3. Appends the file path + timestamp to `~/Dialpad-Design-Artifacts/manifest.md`
4. Prints a reminder about the Design Studio MCP for live component token lookups
...
MANIFEST_DIR="$HOME/Dialpad-Design-Artifacts"
mkdir -p "$MANIFEST_DIR"

TIMESTAMP=$(date +"%Y-%m-%d %H:%M")
echo "- [$TIMESTAMP] $FILE" >> "$MANIFEST_DIR/manifest.md"

echo "[design-studio-export] Design spec logged to $MANIFEST_DIR/manifest.md"
echo "[design-studio-export] Tip: run the Design Studio MCP to validate Dialtone token usage in this spec."
```
...
Each spec write appends a line to `~/Dialpad-Design-Artifacts/manifest.md`:
...
- [Design Studio MCP Setup](../../09_mcps/guides/DP-General-design-studio-setup.md)
